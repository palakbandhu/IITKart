...
exports.getMe = exports.resendRegistrationOtp = exports.verifyRegistrationOtp = exports.resetPassword = exports.verifyOtp = exports.forgotPassword = exports.login = exports.register = void 0;
    ...
    try {
        ...
        if (phone) {
            ...
            if (!/^[0-9]{10}$/.test(cleanPhone)) {
                return next(new AppError_1.AppError(`Phone number must contain exactly 10 digits. Received: "${cleanPhone}"`, 400));
            }
        }
        let dbRole = 'user';
        ...
        const pendingUser = await db_1.default.pendingUser.upsert({
            where: { email },
        ...
        await notificationService_1.notificationService.sendRegistrationOTP(pendingUser.email, otp);
        res.status(201).json({
            success: true,
...
const login = async (req, res, next) => {
    try {
        ...
        if (!user.isVerified) {
            return next(new AppError_1.AppError('Please verify your email to login.', 403));
        }
...
const verifyRegistrationOtp = async (req, res, next) => {
    try {
        const { userId, otp } = req.body;
        const pendingUser = await db_1.default.pendingUser.findFirst({
        ...
        const accessToken = authService_1.authService.generateAccessToken(returnUser.id, returnUser.role);
        const refreshToken = authService_1.authService.generateRefreshToken(returnUser.id);
        res.status(200).json({
            success: true,
            message: 'Email verified successfully',
            data: { user: (0, helpers_1.sanitizeUser)(returnUser), accessToken, refreshToken }
        });
    }
    catch (error) {
        next(error);
...
const resendRegistrationOtp = async (req, res, next) => {
    try {
        ...
        const pendingUser = await db_1.default.pendingUser.findUnique({ where: { id: userId } });
        if (!pendingUser) {
            const existingUser = await db_1.default.user.findUnique({ where: { id: userId } });
            if (existingUser && existingUser.isVerified)
                return next(new AppError_1.AppError('User is already verified', 400));
            return next(new AppError_1.AppError('User not found in pending registrations', 404));
        }
        const otp = authService_1.authService.generateOTP();
        const otpExpiry = new Date(Date.now() + 10 * 60 * 1000);
        await db_1.default.pendingUser.update({
            where: { id: userId },
            data: { otp, otpExpiry }
        });
        await notificationService_1.notificationService.sendRegistrationOTP(pendingUser.email, otp);
